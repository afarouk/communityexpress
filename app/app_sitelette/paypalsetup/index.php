  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="jumbotron">
        ...
        <?php if(in_array($merchant_country, $PPH_DATA->card_countries)){
         //if the merchant is in a PPH Compatible Country
          ?>
        <p class="lead">The fast and easy way to accept credit cards in-store.</p>
        <img src="img/readers/<?php echo $PPH_DATA->$merchant_country->reader_image; ?>" alt="PayPal Credit Card Reader" />
        <p style="font-size:12px;">Simple flat rate pricing starting at <?php echo $PPH_DATA->$merchant_country->rate_standard; ?>*, no hidden cost, and no long term commitments. </p>
        <p>
          <a class="btn btn-primary" href="<?php echo $PPH_DATA->$merchant_country->account_onboard_link; ?>" target="<?php echo $PPH_DATA->$merchant_country->onboard_target; ?>" role="button">
            <?php echo $PPH_DATA->$merchant_country->account_onboard_text; ?>
          </a>
        </p>
        <hr/>
        <p>Already created a PayPal Account for your Business? </p>
        <p style="font-size:12px;">Use the button below to connect it to <?php echo $PARTNER_NAME; ?>: </p>
