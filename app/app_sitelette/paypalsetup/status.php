...
<html lang="en">
  <head>
    ...
    <title><?php echo $PARTNER_NAME; ?> - Connect PayPal Here</title>
  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="jumbotron">
        ...
        <p style="font-size:12px;"><?php echo $paypal_account_details->email; ?></p>
      ...
      <div class="row marketing">
        <div>
          ...
          <?php if(in_array($paypal_account_country, $PPH_DATA->card_countries)){ ?>
          ...
          <?php if(in_array($paypal_account_details->account_type, $PPH_DATA->$paypal_account_country->account_types)){ ?>
          <div class="success-row">
          ...
          <div class="warn-row">
            <h4>PayPal Account Type</h4>
            <p>Please <a href="<?php echo $PPH_DATA->$paypal_account_country->account_upgrade_link; ?>" target="_blank">upgrade your PayPal account</a>.</p>
          </div>
          <?php } ?>


          <?php if($paypal_account_retail_status->status == 'ready' && (in_array('chip', $paypal_account_retail_status->paymentTypes) || in_array('card', $paypal_account_retail_status->paymentTypes))){ ?>
          <div class="success-row">
           ...
           <div style="text-align:center;">
            ...
            <p>You can <a href="<?php echo $PPH_DATA->$paypal_account_country->webstore_link; ?>" target="_blank">order extra PayPal card readers online</a>.</p>
          </div>
          ...
          <div class="warn-row">
            <h4>Physical Card Processing</h4>
            <p>You must <a href="<?php echo $PPH_DATA->$paypal_account_country->pph_onboard_link; ?>" target="_blank">apply for PayPal Here Card Processing</a>.</p>
            <p style="font-size:12px;">If you have already applied for PayPal Here it may take some time to activate. <br/>Please Contact PayPal at: <?php echo $PPH_DATA->$paypal_account_country->support_phone; ?>  to check your application status. </p>
          </div>
          <?php } ?>
          ...
          <?php }else{ //Looks like this Account is not in a valid Country ?>
          <div class="warn-row">
            <h4>PayPal Account Country</h4>
            <p>Sorry.  PayPal Card Processing is only available in (<?php implode(", ", $PPH_DATA->card_countries); ?>) at this time..</p>
          </div>
          <?php } ?>
          ...
          <div id="collapse3" class="panel-collapse collapse">
            ...
            <h5>Refresh Token:</h5>
            <pre><?php $refresh_token; ?></pre>
            <h5>Access Token:</h5>
            <pre><?php $access_token; ?></pre>
            <h5>Retail SDK Token:</h5>
            <?php //lets quickly format the refresh URL for this token
            ...
            ?>
            <pre><?php echo $sdk_token_decoded = '["'.$access_token.'",28800,"'.$refresh_url.'"]'; ?></pre>
            <pre><?=$PP_ENVIRONEMNT_NAME; ?>:<?=base64_encode($sdk_token_decoded); ?></pre>
            <h5>UserInfo Response</h5>
            <pre><?php print_r($paypal_account_details); ?></pre>
            <h5>Retail Status Response</h5>
            <pre><?php print_r($paypal_account_retail_status); ?></pre>
